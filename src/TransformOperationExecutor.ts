...
import { TypeHelpOptions, TypeOptions, ClassTransformOptions, TypeMetadata, ClassConstructor } from './interfaces';
...
export class TransformOperationExecutor {
    ...
    if (Array.isArray(value) || value instanceof Set) {
      ...
      for (const key of keys) {
        ...
        if (!this.options.ignoreDecorators && targetType) {
          if (this.transformationType === TransformationType.PLAIN_TO_CLASS) {
            const exposeMetadata = defaultMetadataStorage.findExposeMetadataByCustomName(
              targetType as ClassConstructor<any>,
              key
            );
            if (exposeMetadata) {
            ...
            const exposeMetadata = defaultMetadataStorage.findExposeMetadata(targetType as ClassConstructor<any>, key);
        ...
        if (targetType && isMap) {
          ...
          const metadata = defaultMetadataStorage.findTypeMetadata(targetType as ClassConstructor<any>, propertyName);
          if (metadata) {
  ...
  private getReflectedType(target: Function, propertyName: string): Function | undefined {
    ...
    const meta = defaultMetadataStorage.findTypeMetadata(target as ClassConstructor<any>, propertyName);
    return meta ? meta.reflectedType : undefined;
  }

  private getKeys(target: Function, object: Record<string, any>): string[] {
    // determine exclusion strategy
    let strategy = defaultMetadataStorage.getStrategy(target as ClassConstructor<unknown>);
    ...
    if (!this.options.ignoreDecorators && target) {
      ...
      if (this.transformationType === TransformationType.PLAIN_TO_CLASS) {
        exposedProperties = exposedProperties.map(key => {
          const exposeMetadata = defaultMetadataStorage.findExposeMetadata(target as ClassConstructor<any>, key);
          if (exposeMetadata && exposeMetadata.options && exposeMetadata.options.name) {
